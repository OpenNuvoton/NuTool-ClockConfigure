...
(function () {
    ...
    var g_NUC_TreeView_Width = 250,
        ...
        g_Dialog_Width,
        g_Dialog_Height,
        g_chipTypes = ["KM1M4BF", "KM1M7AF", "KM1M7BF", "KM1M7CF", "M030G", "M031", "M051", "M0518", "M0519", "M0564", "M2003C", "M2351", "M251", "M261", "M451", "M460", "M480", "MINI51", "NANO100", "NM1200", "NM1500", "NUC029", "NUC100", "NUC200", "NUC400", "NUC505"],
        g_chipType = "",
        ...
        g_clockRegs = [], // to stash the real-time GPIO multiple function control registers
    ...
    function createD3ClockTree(inputModule) {
        ...
        pushRelationshipChildren = function (whichChildren, moduleName) {
            ...
            } else if (whichChildren === 'LIRC' || whichChildren === sLIRC) {
        ...
        populateD3Tree = function (targetNodes) {
            ...
            for (ii = 0, imax = targetNodes.length; ii < imax; ii += 1) {
                ...
                else if (localJsonData.name === 'LIRC' || localJsonData.name === sLIRC) {
                ...
                } else if (localJsonData.name === 'LXT') {
        ...
        if (_.filter(NUTOOL_CLOCK.g_register_map[sPWRCON], function (s) { return s.indexOf(sXTL32K_EN + ':') !== -1; }).length !== 0) {
            rootChildren.push('LXT');
        }
        if (_.filter(NUTOOL_CLOCK.g_register_map[sPWRCON], function (s) { return s.indexOf(sXTL12M_EN + ':') !== -1; }).length !== 0) {
        ...
        function dblclick(d) {
            ...
            if ($.inArray(d.name, rootChildren) === -1) {
                ...
                if (d.name.indexOf('LIRC') === 0 || d.name.indexOf(sLIRC) === 0 || d.name.indexOf(sHIRC) === 0 || d.name.indexOf(sHIRC2) === 0 || d.name.indexOf('HIRC48') === 0 || d.name.indexOf('MIRC') === 0 || d.name.indexOf('LXT') === 0 || d.name.indexOf(sHXT) === 0 || d.name.indexOf('RTC32k') === 0) {
                    $("#tabs").tabs({ active: 0 });
                }
    ...
    function createModuleCanvas(hostDivString, moduleName, defaultValue, selectField, dividerField) {
        ...
        initializeModule = function () {
            // KM1M7CF && SYSTICK: 因為要一次判斷兩個register，所以獨立出來寫
            if (g_chipType.indexOf("KM1M7CF") === 0 && moduleName === 'SYSTICK') {
                if (NUTOOL_CLOCK.g_Module[moduleName][2] !== 'none') {
                ...
                var clksrc = readValueFromClockRegs('CLKSRC').toString();
                var stcksel = readValueFromClockRegs('STCKSEL').toString();
                // TODO: Refine code
                if (clksrc == 1 && stcksel == 0) {
                    selectFieldValue = 0;
                } else if (clksrc == 0 && stcksel == 0) {
                    selectFieldValue = 1;
                } else if (clksrc == 0 && stcksel == 1) {
                    selectFieldValue = 2;
                } else {
                    selectFieldValue = 2;
                }
                    ...
                    for (i = 0, max = moduleSourceInformationArray.length; i < max; i += 1) {
                        if (i === pressedI) {
                            ...
                            if (moduleName !== 'SYSTICK') {
                                ...
                                if (g_chipType.indexOf("KM1M7CF") === 0) {
                                    for (j = 0, maxJ = NUTOOL_CLOCK.g_CLKSEL['CLKSRC'].length; j < maxJ; j += 1) {
                                        fullFieldName = NUTOOL_CLOCK.g_CLKSEL['CLKSRC'][j];
                                        if (fullFieldName.indexOf(inputSource) !== -1 && fullFieldName.indexOf(':') === inputSource.length) {
                                            writeNewValueToClockRegs('CLKSRC', parseInt(fullFieldName.sliceAfterX(':'), 10), '', false);
                                            break;
                                        }
                                    }
                                    for (j = 0, maxJ = NUTOOL_CLOCK.g_CLKSEL['STCKSEL'].length; j < maxJ; j += 1) {
                                        fullFieldName = NUTOOL_CLOCK.g_CLKSEL['STCKSEL'][j];
                                        if (fullFieldName.indexOf(inputSource) !== -1 && fullFieldName.indexOf(':') === inputSource.length) {
                                            writeNewValueToClockRegs('STCKSEL', parseInt(fullFieldName.sliceAfterX(':'), 10), '', false);
                                            break;
                                        }
                                    }
                                } else if (inputSource.indexOf('CPUCLK') !== -1) {
                                    writeNewValueToClockRegs('CLKSRC', 1, sSYST_CSR);
                                }
                                else {
                                    writeNewValueToClockRegs('CLKSRC', 0, sSYST_CSR);
        ...
        if (selectRegister.indexOf('DIV+') !== -1) {
            ...
            selectRegister.indexOf('LIRC') === 0 || selectRegister.indexOf(sLIRC) === 0 ||
            selectRegister.indexOf(sHIRC) === 0 || selectRegister.indexOf(sHIRC2) === 0 ||
            selectRegister.indexOf('MIRC') === 0 || selectRegister.indexOf('LXT') === 0 || selectRegister.indexOf(sHXT) === 0 ||
    ...
    function createModuleOnly(hostDivString, moduleName, defaultValue, selectField) {
        ...
        if (moduleName !== 'SYSTICK') {
            ...
            // 因為KM1M7CF的STCKSEL中，CPUCLK和HXOCLK/128皆為0，所以要用SYST_CTRL的CLKSRC來判斷
            if (g_chipType.indexOf("KM1M7CF") === 0) {
                selectFieldValue = readValueFromClockRegs('CLKSRC').toString();
                if (selectFieldValue == 1) {
                    selectFieldName = 'CPUCLK';
                } else {
                    selectFieldValue = readValueFromClockRegs('STCKSEL').toString();
                    if (selectFieldValue == 0) {
                        selectFieldName = 'HXOCLK/128';
                    } else {
                        selectFieldName = 'HRCCLK1/128';
                    }
                }
            }
            else if (isFieldBe1('CLKSRC')) {
                selectFieldName = 'CPUCLK';
            }
            else {
    ...
    function buildHCLKandPCLKtab(command) {
        ...
        drawHCLKorPCLKdiagram = function (canvasElement, moduleName, dividerField, allowedInput, adjacentInput) {
            var context = g_utility.getContext(canvasElement),
                ...
                initializeModule = function () {
                    if (dividerField !== '') {
                        if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(dividerField)) { // KM1M7
                            selectFieldValue = readValueFromClockRegs(dividerField).toString();
                            for (i = 0, max = NUTOOL_CLOCK.g_CLKSEL[dividerField].length; i < max; i += 1) {
                                if (NUTOOL_CLOCK.g_CLKSEL[dividerField][i].sliceAfterX(':') === selectFieldValue) {
                                    selectFieldName = NUTOOL_CLOCK.g_CLKSEL[dividerField][i].slicePriorToX(':');
                                    break;
                                }
                            }
                            if (selectFieldName.indexOf('/') === -1) {
                                dividerFieldInformation.name = '1/(' + dividerField + ': 1)';
                            }
                            else {
                                dividerFieldInformation.name = '1/(' + dividerField + ': ' + selectFieldName.sliceAfterX('/') + ')';
                            }
                        }
                        else {
                            dividerFieldInformation.name = '1/(' + dividerField + ': ' + readValueFromClockRegs(dividerField) + ' + 1)';
                        }
                        // clear text
                        context.fillStyle = '#FFFFFF';
                        dividerFieldInformation.textLength = context.measureText(dividerFieldInformation.name).width;
                    ...
                    // decide selectFieldName
                    selectFieldName = "";
                    if (moduleName === 'BASECLK') {
                        fullFieldName = 'BASESEL';
                    }
                    else if (moduleName === sHCLK) {
                        if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty('BASESEL')) {
                            fullFieldName = 'CPUCKDIV'; // KM1M7
                        }
                        else {
                        ...
                        if (moduleSourceInformationArray[pressedI].name.indexOf('Disabled') === -1) {
                            for (i = 0, max = moduleSourceInformationArray.length; i < max; i += 1) {
                                if (i === pressedI) {
                                    ...
                                    // update selectField
                                    if (moduleName.indexOf('BASECLK') !== -1) {
                                        selectField = 'BASESEL';
                                        inputSource = moduleSourceInformationArray[i].name.slicePriorToLastX(':');
                                    }
                                    else if (moduleName.indexOf(sHCLK) !== -1) {
                                        if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty('BASESEL')) {
                                            selectField = 'CPUCKDIV'; // KM1M7
                ...
                generateRealModuleFrequency = function () {
                    ...
                    for (i = 0, max = moduleSourceInformationArray.length; i < max; i += 1) {
                        if (moduleSourceInformationArray[i].color === colorForConfiguredByDiagram) {
                            fullFieldName = moduleSourceInformationArray[i].name;
                            if (fullFieldName.indexOf(sHCLK) === -1 && fullFieldName.indexOf('BASECLK') === -1) {
                                if (fullFieldName.indexOf('MHz') !== -1) {
                                    moduleRealFrequency = parseFloat(fullFieldName.sliceAfterX(':'), fullFieldName.length - 3) * 1000000;
                                }
                                else if (fullFieldName.indexOf('kHz') !== -1) {
                    ...
                    if (readValueFromClockRegs(sHCLK_N) !== -1) {
                        if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(sHCLK_N)) { // KM1M7
                            selectFieldValue = readValueFromClockRegs(sHCLK_N).toString();
                            for (j = 0, maxJ = NUTOOL_CLOCK.g_CLKSEL[sHCLK_N].length; j < maxJ; j += 1) {
                                if (NUTOOL_CLOCK.g_CLKSEL[sHCLK_N][j].sliceAfterX(':') === selectFieldValue) {
                                    selectFieldName = NUTOOL_CLOCK.g_CLKSEL[sHCLK_N][j].slicePriorToX(':');
                                    break;
                                }
                            }
                            if (selectFieldName.indexOf('/') !== -1) {
                                moduleRealFrequency = moduleRealFrequency / parseInt(selectFieldName.sliceAfterX('/'), 10);
                            }
                        }
                        else {
                    ...
                    if (fullFieldName !== "") {
                        if (!NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(sPCLK0SEL)) {
                            if (readValueFromClockRegs(sPCLK_S) !== -1) {
                                if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(sPCLK_S)) {
                                    ...
                                    if (selectFieldName.indexOf('/') === -1) {
                                        ...
                                        moduleRealFrequency = fullFieldName.toFloat() / parseInt(selectFieldName.sliceAfterX('/'), 10);
                                    }
                                }
                                else {
                            ...
                            if (readValueFromClockRegs(sPCLK0SEL) > 0) {
                                g_realPCLK0outputClock = fullFieldName.toFloat() / Math.pow(2, readValueFromClockRegs(sPCLK0SEL));
                            }
                            else {
                                g_realPCLK0outputClock = fullFieldName.toFloat();
                            }
                            $("#PCLK0_span_showRealFreq").text(g_realPCLK0outputClock.toHzString());

                            if (readValueFromClockRegs(sPCLK1SEL) > 0) {
                                g_realPCLK1outputClock = fullFieldName.toFloat() / Math.pow(2, readValueFromClockRegs(sPCLK1SEL));
                            }
                            else {
                                g_realPCLK1outputClock = fullFieldName.toFloat();
                            }
                            $("#PCLK1_span_showRealFreq").text(g_realPCLK1outputClock.toHzString());

                            if (readValueFromClockRegs(sPCLK2SEL) > 0) {
                                g_realPCLK2outputClock = fullFieldName.toFloat() / Math.pow(2, readValueFromClockRegs(sPCLK2SEL));
                            }
                            else {
                                g_realPCLK2outputClock = fullFieldName.toFloat();
                            }
            ...
            for (i = 0, max = inputSourceNumber; i < max; i += 1) {
                if (allowedInput[i].indexOf(sHCLK) === -1 && allowedInput[i].indexOf('BASECLK') === -1) {
                    fillTextArray.push(allowedInput[i] + ': ' + decideInputClockFreq(allowedInput[i]).toHzString());
                }
                else {
            ...
            if (inputSourceNumber > 1) {
                ...
                if (dividerField !== '') {
                    ...
                    if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(dividerField)) { // KM1M7
                        context.fillText('1/(' + dividerField + ': 0)', x, y);
                        dividerPartLength += context.measureText('1/(' + dividerField + ': 0)').width;
                        // stash the dividerField information
                        dividerFieldInformation = {
                            name: '1/(' + dividerField + ': 0)',
                            x: x,
                            y: y,
                            textLength: context.measureText('1/(' + dividerField + ': 0)').width,
                            color: 'black',
                            lastColor: 'black'
                        };
                    }
                ...
                if (dividerField !== '') {
                    x = canvas_startPointX + arrowLineLength;
                    if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(dividerField)) { // KM1M7
                        context.fillText('1/(' + dividerField + ': 0)', x, y + 18 / 2 - 2);
                        dividerPartLength += context.measureText('1/(' + dividerField + ': 0)').width;
                        // stash the dividerField information
                        dividerFieldInformation = {
                            name: '1/(' + dividerField + ': 0)',
        ...
        if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty('BASESEL')) {
            g_enabledBaseClocks.push('BASECLK'); // KM1M7
        }
        ...
        if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty('BASESEL')) {
            drawHCLKorPCLKdiagram($HCLK_canvas[0], 'BASECLK', '', allowedInput, NUTOOL_CLOCK.g_CLKSEL.CPUCKDIV);

            allowedInput = [];
            selectField = 'CPUCKDIV';
            if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(selectField)) {
                for (j = 0, maxJ = NUTOOL_CLOCK.g_CLKSEL[selectField].length; j < maxJ; j += 1) {
                    allowedInput.push(NUTOOL_CLOCK.g_CLKSEL[selectField][j].slicePriorToX(':'));
                }
            ...
            $tab_3[0].appendChild(ce("canvas", "HCLK_canvas1"));
            $HCLK_canvas1 = $("#HCLK_canvas1");
            $HCLK_canvas1[0].setAttribute('style', 'z-index:1; position:absolute; background-color: #FFFFFF; left:400px; top:' + 85 + 'px; border: 1px solid black;');
            drawHCLKorPCLKdiagram($HCLK_canvas1[0], sHCLK, '', allowedInput);
        }
        else {
        ...
        if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(sPCLK0SEL)) {
            ...
            if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(sPCLK1SEL)) {
                ...
                $PCLK1_canvas[0].setAttribute('style', 'z-index:1; position:absolute; background-color: #FFFFFF; left:300px; top:' + HCLKblockHeight + 'px; border: 1px solid black;');
            ...
            if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(sPCLK2SEL)) {
                ...
                $PCLK2_canvas[0].setAttribute('style', 'z-index:1; position:absolute; background-color: #FFFFFF; left:570px; top:' + HCLKblockHeight + 'px; border: 1px solid black;');
    ...
    function buildPLLclockTab(command) {
        ...
        if (g_chipType === "NUC400") {
            ...
            if (g_realPLLoutputClock === 0) {
                localRealPLLoutputClock = 100000000;
            }
        }
        ...
        buildPLLCTLtable = function (expectationFrequency, bClickFromRegister, inaccuracy, registerName) {
            ...
            if (typeof (inaccuracy) !== 'undefined') {
                ...
                if (g_chipType.indexOf("KM1M7CF") === 0) {
                    ...
                    if ((isFieldBe1(sOSC22M_EN) || isFieldBe1('HIRC1EN')) &&
                        g_chipType.indexOf("KM1M7") === -1 && g_chipType.indexOf("KM1M4") === -1) {
                ...
                formulaType = "General";
                for (i = 0, max = NUTOOL_CLOCK.g_register_map[registerName].length; i < max; i += 1) {
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'FB_DV:5-0') {
                        formulaType = "NANO100AN_BN";
                        break;
                    }
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'PLL_MLP:5-0') {
                        formulaType = "NANO112_103";
                        break;
                    }
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'PLLREMAP:20') {
                        formulaType = "NUC400";
                        break;
                    }
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'FBDIV:6-0') {
                        formulaType = "NUC505(PLL)";
                        break;
                    }
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'FRAC:31-20') {
                        formulaType = "NUC505(APLL)";
                        break;
                    }
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'FBDIV:5-0') {
                        formulaType = "M251";
                        break;
                    }
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'PLLMUL:6-0') {
                        formulaType = "KM1M4";
                        break;
                    }
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'PLLMUL:5-0') {
                        formulaType = "KM1M7";
                        break;
                    }
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'STBSEL:23') {
                        formulaType = "M460(PLL)";
                        break;
                    }
                    if (NUTOOL_CLOCK.g_register_map[registerName][i] === 'FRDIV:27-16') {
                        formulaType = "M460(PLLFN)";
                        break;
                    }
                }
                if (g_chipType.indexOf("M480") === 0 || g_chipType === 'M2351' || g_chipType === 'M2354' || g_chipType === 'M261') {
                    formulaType = "M480";
                }
                ...
                if (formulaType === "NANO100AN_BN") {
                    ...
                    for (i = 0, max = FIN.length; i < max; i += 1) {
                        ...
                        for (FB_DV = 10, maxFB_DV = 81; FB_DV < maxFB_DV; FB_DV += 1) {
                            for (IN_DV = 0, maxIN_DV = Math.pow(2, 2); IN_DV < maxIN_DV; IN_DV += 1) {
                                if (IN_DV === 0) {
                                    NR = 1;
                                }
                                ...
                                inputClockFreq1 = inputClockFreq * FB_DV / (NR * 2);
                                outputClockFreq = inputClockFreq1 / 2;
                                //test it with the constrains
                                if (IN_DV !== 1 &&
                                    (inputClockFreq / NR >= minFIN) &&
                                    (inputClockFreq / NR <= maxFIN) &&
                                    inputClockFreq1 >= minFCO &&
                                    inputClockFreq1 <= maxFCO &&
                                    outputClockFreq >= minFOUT &&
                                    outputClockFreq <= maxFOUT) {
                                    calculatedinaccuracy = Math.abs(outputClockFreq - expectationFrequency) / expectationFrequency * 100;
                                    if (calculatedinaccuracy <= inaccuracy) {
                                        if (((IN_DV << 8) + FB_DV) === recordedPLLCTL &&
                ...
                else if (formulaType === "KM1M7") {
                    // KM1M7AF or KM1M7BF
                    if (NUTOOL_CLOCK.g_register_map['CHIPPLLCTR2'][0] === 'PLLIDIV:9-8') {
                        for (i = 0, max = FIN.length; i < max; i += 1) {
                            inputSource = FIN[i];
                            if (inputSource === sHXT) {
                                inputClockFreq = parseFloat($('#' + sXTL12M_EN + '_input').val()) * 1000000; // MHz
                            ...
                            for (FB_DV = 10, maxFB_DV = Math.pow(2, 6); FB_DV < maxFB_DV; FB_DV += 1) {
                                for (IN_DV = 0, maxIN_DV = Math.pow(2, 2); IN_DV < maxIN_DV; IN_DV += 1) {
                                    for (OUT_DV = 0, maxOUT_DV = Math.pow(2, 1); OUT_DV < maxOUT_DV; OUT_DV += 1) {
                                        if (IN_DV === 0) {
                                            NR = 1;
                                        ...
                                        NO = OUT_DV + 1;
                                        inputClockFreq1 = inputClockFreq * FB_DV / NR;
                                        outputClockFreq = inputClockFreq1 / NO;
                                        //test it with the constrains
                                        if (IN_DV !== 1 &&
                                            (inputClockFreq / NR >= minFIN) &&
                                            (inputClockFreq / NR <= maxFIN) &&
                                            ...
                                            calculatedinaccuracy = Math.abs(outputClockFreq - expectationFrequency) / expectationFrequency * 100;
                                            if (calculatedinaccuracy <= inaccuracy) {
                                                if (((OUT_DV << 6) + (IN_DV << 8) + FB_DV) === recordedPLLCTL &&
                                                    inputSource === recordedIntputSource) {
                                                    aaDataArray.unshift([]);
                                                    aaDataArray[0][0] = '';
                                                    aaDataArray[0][1] = '0x' + decimalToHex((OUT_DV << 6) + (IN_DV << 8) + FB_DV, 4).toUpperCase();
                                                    aaDataArray[0][2] = inputSource;
                                                    aaDataArray[0][3] = (outputClockFreq / 1000000).toFixed(4) + 'MHz';
                                                    aaDataArray[0][4] = calculatedinaccuracy.toFixed(4) + '%';
                                                    ...
                                                    aaDataArray[aaDataArray.length] = [];
                                                    aaDataArray[aaDataArray.length - 1][0] = '';
                                                    aaDataArray[aaDataArray.length - 1][1] = '0x' + decimalToHex((OUT_DV << 6) + (IN_DV << 8) + FB_DV, 4).toUpperCase();
                                                    aaDataArray[aaDataArray.length - 1][2] = inputSource;
                                                    aaDataArray[aaDataArray.length - 1][3] = (outputClockFreq / 1000000).toFixed(4) + 'MHz';
                                                    aaDataArray[aaDataArray.length - 1][4] = calculatedinaccuracy.toFixed(4) + '%';
                                                }
                    ...
                    // KM1M7CF
                        ...
                        for (i = 0, max = FIN.length; i < max; i += 1) {
                            ...
                            for (FB_DV = 10, maxFB_DV = Math.pow(2, 6); FB_DV < maxFB_DV; FB_DV += 1) {
                                for (IN_DV = 0, maxIN_DV = Math.pow(2, 1); IN_DV < maxIN_DV; IN_DV += 1) {
                                    for (OUT_DV = 0, maxOUT_DV = Math.pow(2, 1); OUT_DV < maxOUT_DV; OUT_DV += 1) {
                                        if (IN_DV === 0) {
                                            NR = 1;
                                        }
                    ...
                    for (i = 0, max = FIN.length; i < max; i += 1) {
                        ...
                        if (inputSource === sHIRC) {
                            ...
                            inputClockFreq = NUTOOL_CLOCK.g_HIRCfrequency / parseInt(inputSource.sliceAfterX('/'), 10);
                        }
                        else {
                            inputClockFreq = parseFloat($('#' + sXTL12M_EN + '_input').val()) * 1000000; // MHz
                        }
                ...
                if (formulaType === "NANO100AN_BN" || formulaType === "NANO112_103" ||
                    formulaType === "KM1M4" || formulaType === "KM1M7") {
                    ...
                    $("#PLLCTLtable tbody").delegate("tr td:first-child", "click", function (event) {
                        ...
                        for (i = 0, max = aTrs.length; i < max; i++) {
                            if ($(aTrs[i]).hasClass('row_selected')) {
                                ...
                                else if (formulaType === "KM1M4" || formulaType === "KM1M7") {
                                    PLLCTLvalue = aTrs[i].childNodes[1].innerText;
                                    g_realPLLoutputClock = aTrs[i].childNodes[3].innerText;
                                    g_realPLLoutputClock = parseFloat(g_realPLLoutputClock.slicePriorToX('MHz')) * 1000000;
                                    // OUT_DV, IN_DV, FB_DV
        ...
        $(".cb-enable").click(function () {
            ...
            if (currentHtmlFor.indexOf('PD') !== -1 || currentHtmlFor.indexOf('PLL2CKEN') !== -1) {
                ...
                if (currentHtmlFor.indexOf('PLL2CKEN') !== -1) {
                    ...
                    writeNewValueToClockRegs('PLLEN', 1); // KM1M7
                }
        ...
        $(".cb-disable").click(function () {
            ...
            if (currentHtmlFor.indexOf('PD') !== -1 || currentHtmlFor.indexOf('PLL2CKEN') !== -1) {
                ...
                if (currentHtmlFor.indexOf('PLL2CKEN') !== -1) {
                    ...
                    writeNewValueToClockRegs('PD', 1);
                    writeNewValueToClockRegs('OE', 1);
                    if (g_clockRegs['CHIPPLLCTR2'] != undefined) { // Reset CHIPPLLCTR2 value for KM1M7
                        g_clockRegs['CHIPPLLCTR2'] = NUTOOL_CLOCK.g_register_map_default['CHIPPLLCTR2'];
                    }
    ...
    function buildRefClockTab(command) {
        ...
        chainEffectHandler2 = function () {
            if (isFieldBe1(sXTL32K_EN)) {
                NUTOOL_CLOCK.g_Module.RTC[0] = 'LXT';
            }
            else {
                if (g_chipType.indexOf("KM1M7CF") === 0) {
                    NUTOOL_CLOCK.g_Module.RTC[0] = sLIRC;
                } else {
                    NUTOOL_CLOCK.g_Module.RTC[0] = 'LIRC';
                }
            }
        };
        ...
        // build Base Clocks
        if (NUTOOL_CLOCK.g_LIRCfrequency !== 0) {
            if (_.filter(NUTOOL_CLOCK.g_register_map[sPWRCON], function (s) { return s.indexOf(sOSC10K_EN + ':') !== -1; }).length !== 0) {
                if (g_chipType.indexOf("KM1M7CF") === 0) {
                    buildOneSetOfClock($tab_1, sOSC10K_EN, sLIRC, NUTOOL_CLOCK.g_LIRCfrequency / 1000, 'kHz', 'text', true);
                } else {
                    buildOneSetOfClock($tab_1, sOSC10K_EN, 'LIRC', NUTOOL_CLOCK.g_LIRCfrequency / 1000, 'kHz', 'text', true);
                }
            }
            else {
                if (g_chipType.indexOf("KM1M7CF") === 0) {
                    ...
                    buildOneSetOfClock($tab_1, sOSC10K_EN, 'LIRC', NUTOOL_CLOCK.g_LIRCfrequency / 1000, 'kHz', 'alwaysEnabled', true);
                }
            }
        }
        if (NUTOOL_CLOCK.g_HIRCfrequency !== 0) {
            if (NUTOOL_CLOCK.g_HIRCfrequencyArray.length === 0) {
                if (_.filter(NUTOOL_CLOCK.g_register_map[sPWRCON], function (s) { return s.indexOf(sOSC22M_EN + ':') !== -1; }).length !== 0) {
        ...
        if (NUTOOL_CLOCK.g_HIRC2frequency !== 0) {
            if (NUTOOL_CLOCK.g_HIRC2frequencyArray.length === 0) {
                if (_.filter(NUTOOL_CLOCK.g_register_map[sPWRCON], function (s) { return s.indexOf(sOSC22M2_EN + ':') !== -1; }).length !== 0) {
        ...
        if (_.filter(NUTOOL_CLOCK.g_register_map[sPWRCON], function (s) { return s.indexOf(sXTL32K_EN + ':') !== -1; }).length !== 0) {
            buildOneSetOfClock($tab_1, sXTL32K_EN, 'LXT', '32.768', 'kHz', 'text', true);
        }
        ...
        if (NUTOOL_CLOCK.g_RTC32kfrequency !== 0) {
            ...
            opt = window.document.createElement("option");
            try { opt.id = opt.innerHTML = opt.value = 'LXT'; } catch (err) { }
            $("#RTC32kEN_select")[0].appendChild(opt);
            opt = window.document.createElement("option");
            try { opt.id = opt.innerHTML = opt.value = 'LIRC'; } catch (err) { }
            $("#RTC32kEN_select")[0].appendChild(opt);

            if (isFieldBe1('RTCSEL')) {
                $("#RTC32kEN_select").val('LIRC');
            }
            else {
                $("#RTC32kEN_select").val('LXT');
            }
        ...
        if (_.filter(NUTOOL_CLOCK.g_register_map[sPWRCON], function (s) { return s.indexOf(sXTL12M_EN + ':') !== -1; }).length !== 0) {
            if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(sHXTWAIT)) {   // KM1M7
                if (g_realHXToutputClock > 0) {
                    buildOneSetOfClock($tab_1, sXTL12M_EN, sHXT, g_realHXToutputClock / 1000000, 'MHz (' + NUTOOL_CLOCK.g_HXTRange + ')', 'input_select', true);
                }
                else {
                    buildOneSetOfClock($tab_1, sXTL12M_EN, sHXT, NUTOOL_CLOCK.g_HXTfrequency / 1000000, 'MHz (' + NUTOOL_CLOCK.g_HXTRange + ')', 'input_select', true);
                }
                for (i = 0, max = NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT].length; i < max; i += 1) {
                    opt = window.document.createElement("option");
                    try { opt.id = opt.innerHTML = opt.value = NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT][i].slicePriorToX(':'); } catch (err) { }
                    $("#" + sXTL12M_EN + "_select")[0].appendChild(opt);
                }

                selectFieldValue = readValueFromClockRegs(sHXTWAIT).toString();
                for (i = 0, max = NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT].length; i < max; i++) {
                    if (NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT][i].sliceAfterX(':') === selectFieldValue) {
                        $("#" + sXTL12M_EN + "_select").val(NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT][i].slicePriorToX(':'));
                    }
                }
                $("#" + sXTL12M_EN + "_select").on('change', function () {
                    for (i = 0, max = NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT].length; i < max; i++) {
                        if (NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT][i].slicePriorToX(':') === $(this).val()) {
                            writeNewValueToClockRegs(sHXTWAIT, NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT][i].sliceAfterX(':'));
                        }
                    }
                });
            }
    ...
    function decideInputClockFreq(inputSource) {
        ...
        if (inputSource.indexOf('DIV') !== -1) {
            ...
            dividerValue = parseInt(inputSource.sliceAfterX('/'), 10);
        }
        else {
            dividerValue = 1;
        }

        if (inputSource.indexOf('LIRC') === 0 || inputSource.indexOf(sLIRC) === 0) {
        ...
        else if (inputSource.indexOf('LXT') === 0) {
    ...
    function decideNewChipType(newPartNumber_package) {
        ...
        else if (newPartNumber_package.indexOf('KM1M4BF') === 0) {
            newChipType = 'KM1M4BF';
        }
    ...
    function isFieldBe1(fieldName, specificRegister) {
        ...
        fieldAlwaysBe1[sOSC10K_EN] = ['M030G_31G', 'KM1M7CF'];
        fieldAlwaysBe1[sOSC22M_EN] = ['KM1M4BF', 'KM1M7AF', 'KM1M7BF', 'KM1M7CF'];
        fieldAlwaysBe1[sOSC22M2_EN] = ['KM1M7CF'];
    ...
    function initializationByTest() {
        ...
        else if (g_chipType.indexOf('KM1M4') === 0 || g_chipType.indexOf('KM1M7') === 0) {
            if (g_realPLLoutputClock === 0) {
                g_realPLLoutputClock = 100000000;
            }
        }
        else {
            if (g_realPLLoutputClock === 0) {
    ...
    function reportBaseClockFrequencies() {
        ...
        if (g_realLIRCoutputClock > 0) {
            if (g_chipType.indexOf("KM1M7CF") === 0) {
                ...
                reportMessage = "LIRC: " + g_realLIRCoutputClock.toHzString();
            }
        }
    ...
    function retrieveGenerateCodeContent(stage) {
        ...
        if (stage === 0) {
            concatenate_generated_code_begin();
        }
        else if (stage === 1) {
            g_generateCodeContent = "";
            concatenate_generated_code_internal('functionalTest');
        }
        else if (stage === 2) {
            concatenate_generated_code_internal('partial');
        }
        else if (stage === 3) {
            ...
            concatenate_generated_code_end();
    ...
    function readValueFromClockRegs(fieldName) {
        var i,
            max,
            j,
            maxJ,
    ...
    function concatenate_generated_code_begin(command) {
        ...
        generateMainCode = function () {
            ...
            for (i = 0, max = beginningClockRegs.length; i < max; i += 1) {
                ...
                if (beginningClockRegs[i] === sPWRCON) {
                    ...
                    if (isFieldBe1(sXTL12M_EN)) {
                        ...
                        if (g_chipType.indexOf('KM1M4') === 0 || g_chipType.indexOf('KM1M7') === 0) {
                            theCode1 = '';  // new API doesn't have "CLK_DisableHXO()"
                        }
                    }
                    if (isFieldBe1('HIRC1EN')) {
                ...
                if (beginningClockRegs[i] === sPLLCON &&
                    ...
                    if (isFieldBe1('PLLEN') &&
                        _.filter(NUTOOL_CLOCK.g_register_map.CHIPPLLCTR1, function (s) { return s.indexOf('PLLEN' + ':') !== -1; }).length !== 0) {
                        if (g_chipType.indexOf('KM1M7C') === 0) {
                            theCode += '    /* Disable ' + sPLL + ' circuit */\n    CLK_DisablePLL();\n\n';
                            theCode += '    CLK->CHIPPLLCTR1 = 0xA53C0002U;\n';
                            theCode += '    ' + currentNode + ' = 0x' + decimalToHex(g_clockRegs[beginningClockRegs[i]]).toUpperCase() + 'U;\n';
                            // 最前面放CHIPPLLCTRL1.bit2的值 (HXOCLK:0x04 / HRCCLK1:0x00)
                            theCode += '    CLK->CHIPPLLCTR1 = 0xA53C000' + ((g_clockRegs['CHIPPLLCTR1'] << 29) >>> 31) * Math.pow(2, 2) + 'U | CLK->CHIPPLLCTR1 | (1U << CLK_CHIPPLLCTR1_PLLEN_Pos);\n\n';
                        } else if (g_chipType.indexOf('KM1M4B') === 0) {
                            theCode += '    /* Disable ' + sPLL + ' circuit */\n    CLK_DisablePLL();\n\n';
                            theCode += '    /* Set ' + sPLL + ' frequency */\n';
                            theCode += '    ' + currentNode + ' = 0x' + decimalToHex(g_clockRegs[beginningClockRegs[i]]).toUpperCase() + 'UL;\n';
                            theCode += '    CLK->CHIPPLLCTR1 = 0xA53C0001UL;\n\n';
                        } else {
                            theCode += '    /* Set ' + sPLL + ' frequency */\n';
                            theCode += '    ' + currentNode + ' = 0x' + decimalToHex(g_clockRegs[beginningClockRegs[i]]).toUpperCase() + 'UL;\n';
                            if (g_chipType.indexOf('KM1M7A') === 0) {
                                theCode += '    CLK->CHIPPLLCTR1 = 0xA53C0003UL;\n\n';
                            } else if (g_chipType.indexOf('KM1M7B') === 0) {
                                theCode += '    CLK->CHIPPLLCTR1 = 0xA53C0001UL;\n\n';
                            } else {
                                theCode += '    CLK->CHIPPLLCTR1 = 0xA53C0001UL;\n\n';
                            }
                        }
                        if (g_chipType.indexOf('KM1M7') === 0) {
                            theCode += '    /* Waiting for ' + sPLL + ' operating ready */\n    CLK_WaitClockReady(CLK_CHIPCKSTAT_PLLOK_Msk);\n\n';
                        } else if (g_chipType.indexOf('KM1M4') === 0) {
            ...
            if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty('BASESEL')) {
                ...
                if (g_chipType.indexOf("KM1M7CF") === 0) {
                    if (selectFieldName.indexOf('HRCCLK1') !== -1) {
                        theCode += '    /* Set CPUCLK clock */\n    CLK_SetCPUCLK(CLK_BASESEL_HRC1' + ', CLK_CPUCKDIV_';
                    } else if (selectFieldName.indexOf('HRCCLK2') !== -1) {
                        theCode += '    /* Set CPUCLK clock */\n    CLK_SetCPUCLK(CLK_BASESEL_HRC2' + ', CLK_CPUCKDIV_';
                    } else if (selectFieldName.indexOf('HXOCLK') !== -1) {
                        theCode += '    /* Set CPUCLK clock */\n    CLK_SetCPUCLK(CLK_BASESEL_HXO' + ', CLK_CPUCKDIV_';
                    } else if (selectFieldName.indexOf('PLLCLK') !== -1) {
                        theCode += '    /* Set CPUCLK clock */\n    CLK_SetCPUCLK(CLK_BASESEL_PLL' + ', CLK_CPUCKDIV_';
                    }
                } else if (g_chipType.indexOf('KM1M4') === 0 || g_chipType.indexOf('KM1M7') === 0) {
                    theCode += '    /* Set CPUCLK clock */\n    CLK_SetCPUCLK(CLK_BASESEL_' + selectFieldName.slicePriorToX('CLK') + ', CLK_CPUCKDIV_';
                }
                else {
                    theCode += '    /* Set BASECLK clock */\n    CLK_SelectBASECLK(SELECT_' + selectFieldName + ');\n\n';
                    theCode += '    /* Set CPUCLK clock */\n    CLK_SetCPUCLK(CPUCLK_DIV_';
                }

                fullFieldName = 'CPUCKDIV';
                selectFieldValue = readValueFromClockRegs(fullFieldName).toString();
                if (selectFieldValue !== -1 && NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(fullFieldName)) {
                    for (i = 0, max = NUTOOL_CLOCK.g_CLKSEL[fullFieldName].length; i < max; i += 1) {
                        if (NUTOOL_CLOCK.g_CLKSEL[fullFieldName][i].sliceAfterX(':') === selectFieldValue) {
                ...
                theCode += selectFieldName.slicePriorToX(':').sliceAfterX('/') + ');\n\n';
            }
            else {
            ...
            if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(sPCLK0SEL)) {
                ...
                if (sPCLK_S.indexOf('IOCKDIV') === 0) {
                    theCode += '    /* Set IOCLK clock */\n';
                    if (g_chipType.indexOf('KM1M4') === 0 || g_chipType.indexOf('KM1M7') === 0) {
                        theCode += '    CLK_SetIOCLK(CLK_IOCKDIV_';
                    }
                    else {
                        theCode += '    CLK_SetIOCLK(IOCLK_DIV_';
                    }
                    fullFieldName = sPCLK_S;
                    selectFieldValue = readValueFromClockRegs(fullFieldName).toString();
                    if (selectFieldValue !== -1 && NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(fullFieldName)) {
        ...
        if (g_chipType === "M451") {
            ...
            beginningClockRegs = [sPWRCON, sPLLCON];
            sIncludeHeaderFile = '#include "NuMicro.h"';
            enableFieldDefines = ['none', 'none', 'none', 'CLK_EnableXtalRC(CLK_HXOCLK_ON, CLK_WCYCLE_' + _.filter(NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT], function (s) { return s.indexOf(":" + readValueFromClockRegs(sHXTWAIT).toString()) !== -1; })[0].toString().slicePriorToX(':') + ');', 'none', 'none'];
            statusFieldDefines = ['none', 'none', 'none', 'CLK_WaitClockReady(CLK_STABLE_HXOCLK);', 'none', 'none', 'none'];
        }
        else if (g_chipType.indexOf('KM1M7AF') === 0 || g_chipType.indexOf('KM1M7BF') === 0) {
            beginningClockRegs = [sPWRCON, sPLLCON];
            sIncludeHeaderFile = '#include "NuMicro.h"';
            enableFieldDefines = ['none', 'none', 'none', 'CLK_EnableXtalRC(CLK_CHIPCKCTR_HXOEN_Msk, CLK_WCYCLE_' + _.filter(NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT], function (s) { return s.indexOf(":" + readValueFromClockRegs(sHXTWAIT).toString()) !== -1; })[0].toString().slicePriorToX(':') + ');', 'none', 'none'];
            statusFieldDefines = ['none', 'none', 'none', 'CLK_WaitClockReady(CLK_CHIPCKSTAT_HXOOK_Msk);\n\n    /* Disable ' + sPLL + ' circuit */\n    CLK_DisablePLL();', 'none', 'none', 'none'];
        }
        else if (g_chipType.indexOf("KM1M7CF") === 0) {
            beginningClockRegs = [sPWRCON, sPLLCON];
            sIncludeHeaderFile = '#include "NuMicro.h"';
            enableFieldDefines = ['none', 'none', 'none', 'CLK_EnableXtalRC(CLK_HXOCLK_ON, CLK_WCYCLE_' + _.filter(NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT], function (s) { return s.indexOf(":" + readValueFromClockRegs(sHXTWAIT).toString()) !== -1; })[0].toString().slicePriorToX(':') + ');', 'none', 'none'];
            statusFieldDefines = ['none', 'none', 'none', 'CLK_WaitClockReady(CLK_CHIPCKSTAT_HXOOK_Msk);', 'none', 'none', 'none', 'none'];
        }
        ...
        if (g_svgGroup !== null) {
            ...
            if (isFieldBe1(sOSC10K_EN) && g_realLIRCoutputClock > 0) {
                if (g_chipType.indexOf("KM1M7CF") === 0) {
                    ...
                    tableData = {};
                    tableData.module = 'LIRC';
                    tableData.busClock = g_realLIRCoutputClock.toHzString();
                    tableData.engineClock = '';
                    tableDataArray.push(tableData);
                    g_clockRegsString += 'LIRC:' + g_realLIRCoutputClock.toHzString() + '\n';
                }
            }
        ...
        if (g_bHasBSPtoSupport) {
            ...
            if (command === 'default') {
                ...
                theCode3 = concatenate_generated_code_internal('modularizeCode');
            ...
            // the main part
            g_clockRegsString += '\n';
            if (typeof NUTOOL_PER === 'undefined' && g_chipType !== 'NUC505' && g_chipType.indexOf('KM1M4') !== 0 && g_chipType.indexOf('KM1M7') !== 0) {
            ...
            if (bChecked) {
                ...
                if (currentNode === sCLKO) {
                    ...
                    if (g_chipType === 'NANO100AN' || g_chipType === 'NANO100BN') {
                        ...
                        theString = '    CLK_EnableCKO(' + theString1.slicePriorToX(',') + ');\n'; // 去掉MODULE_NoMsk
                        theString2 = '    CLK_DisableCKO();\n';
                    }
                    else {
    ...
    function concatenate_generated_code_HCLK() {
        ...
        if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty('BASESEL')) {
            g_clockRegsString += '\n    /* Check more... */\n';
            selectField = sHXTWAIT;
            if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(selectField)) {
                for (i = 0, max = NUTOOL_CLOCK.g_CLKSEL[selectField].length; i < max; i += 1) {
                    $("#" + sXTL12M_EN + "_select").val(NUTOOL_CLOCK.g_CLKSEL[sHXTWAIT][i].slicePriorToX(':'));
                    g_clockRegsString += '    CLK_EnableHXO(HXO_WCYCLE_' + $("#" + sXTL12M_EN + "_select").val() + ');\n';
                }
                g_clockRegsString += '    CLK_DisableHXO();\n';
            }
            ...
            for (i = 0, max = NUTOOL_CLOCK.g_CLKSEL[selectField].length; i < max; i += 1) {
                ...
                selectFieldValue = readValueFromClockRegs(fullFieldName).toString();
                if (selectFieldValue !== -1 && NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(fullFieldName)) {
                    for (j = 0, maxJ = NUTOOL_CLOCK.g_CLKSEL[fullFieldName].length; j < maxJ; j += 1) {
                        if (NUTOOL_CLOCK.g_CLKSEL[fullFieldName][j].sliceAfterX(':') === selectFieldValue) {
                            selectFieldName = NUTOOL_CLOCK.g_CLKSEL[fullFieldName][j].slicePriorToX(':');
                            break;
                        }
                    }
                }
                g_clockRegsString += '    CLK_SelectBASECLK(SELECT_' + selectFieldName + ');\n';
            }
            ...
            if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(selectField)) {
                ...
                for (i = 0, max = NUTOOL_CLOCK.g_CLKSEL[selectField].length; i < max; i += 1) {
                    ...
                    g_clockRegsString += '    CLK_SetCPUCLK(CPUCLK_DIV_' + selectFieldName.slicePriorToX(':').sliceAfterX('/') + ');\n';
                }
            }
            ...
            if (NUTOOL_CLOCK.g_CLKSEL.hasOwnProperty(selectField)) {
                ...
                for (i = 0, max = NUTOOL_CLOCK.g_CLKSEL[selectField].length; i < max; i += 1) {
                    ...
                    g_clockRegsString += '    CLK_SetIOCLK(IOCLK_DIV_' + selectFieldName.slicePriorToX(':').sliceAfterX('/') + ');\n';
                }
            }
        }
        else {
    ...
    function getFieldDefine(currentNode, selectorName, dividerName) {
        ...
        if (returnValue !== -1) {
            // KM1M7CF && SYSTICK: 因為要一次判斷兩個register，所以獨立出來寫
            if (g_chipType.indexOf("KM1M7CF") === 0 && currentNode === 'SYSTICK') {
                var clksrc = readValueFromClockRegs('CLKSRC').toString();
                var stcksel = readValueFromClockRegs('STCKSEL').toString();
                // TODO: Refine code
                if (clksrc == 1 && stcksel == 0) {
                    fullFieldName = 'CPUCLK';
                } else if (clksrc == 0 && stcksel == 0) {
                    fullFieldName = 'HXOCLK/128';
                } else if (clksrc == 0 && stcksel == 1) {
                    fullFieldName = 'HRCCLK1/128';
                } else {
                    fullFieldName = 'CPUCLK';
                }
            } else if (!(currentNode === 'SYSTICK' && isFieldBe1('CLKSRC'))) {
            ...
            if (g_bHXTorLXT && fullFieldName.indexOf('XT') !== -1) {
                if (g_chipType === 'MINI57') {
                    fullFieldName = fullFieldName.replace(sHXT, 'EXT').replace('LXT', 'EXT');
                }
                else {
                    fullFieldName = fullFieldName.replace(sHXT, 'XTAL').replace('LXT', 'XTAL');
                }
            }
            if (g_chipType === 'MINI57') {
            ...
            else if (g_chipType.indexOf("KM1M7CF") === 0 && currentNode === 'SYSTICK') {
                if (fullFieldName.indexOf('CPUCLK') === 0) {
                    returnString = "CLK_SYSTICK_CLK_CPU";
                } else if (fullFieldName.indexOf('HXOCLK/128') === 0) {
                    returnString = "CLK_SYSTICK_CLK_HXO_DIV128";
                } else if (fullFieldName.indexOf('HRCCLK1/128') === 0) {
                    returnString = "CLK_SYSTICK_CLK_HRCCLK1_DIV128";
                } else {
                    returnString = "CLK_SYSTICK_CLK_CPU";
                }
            }
            else if (g_chipType.indexOf("KM1M7CF") === 0 && currentNode === 'CLKOUT') {
                if (fullFieldName.indexOf('HRCCLK1/8') === 0) {
                    returnString = "CLK_CLKOUTSEL_HRCCLK1_DIV8";
                } else if (fullFieldName.indexOf('HXOCLK/8') === 0) {
                    returnString = "CLK_CLKOUTSEL_HXOCLK_DIV8";
                } else if (fullFieldName.indexOf('PLLCLK/256') === 0) {
                    returnString = "CLK_CLKOUTSEL_PLLCLK_DIV256";
                } else {
                    returnString = "CLK_CLKOUTSEL_HRCCLK1_DIV8";
                }
            }
            ...
            if ((g_chipType.indexOf("M25") === 0 || g_chipType.indexOf('M030') === 0 || g_chipType.indexOf('M031') === 0) &&
                ...
                if (returnString.indexOf('LIRC') !== -1) {
    ...
    function generateCode(mode) {
        ...
        buildReviewReportTable = function () {
            ...
            tableDataArray = concatenate_generated_code_begin();
    ...
    function concatenate_generated_code() {
        ...
        concatenate_generated_code_internal();
        // in the end
        concatenate_generated_code_end();
    }
    ...
    function concatenate_generated_modularizedCode() {
        // in the beginning
        concatenate_generated_code_begin('modularizeCode');
        // in the end
        concatenate_generated_code_end();
    }
    ...
    // function loadClockConfigureTool_fromSelectionGuide() {
    //     var oldChipType;
    ...
    function calculatePLLfrequency(Fin, regValue) {
        ...
        if (g_chipType.indexOf("KM1M") === 0) { // NTCJ series
            if (PLLIDIV == undefined || PLLMUL == undefined) {
                ...
                console.warn('calculatePLLfrequency: Unknown g_chipType');
            }
        } else {
