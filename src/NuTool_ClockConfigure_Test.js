...
function loadCorrespondingJS(mcu, oldChipType) {
	...
	chipType = S.eval('NUTOOL_CLOCK.decideNewChipType("' + mcu +'");');
	...
	for (x = 0, maxX = targetChips.length; x < maxX; x = x + 1) {
		...
		if (bManipulateD3Nodes) {
			test("Manipulate the Module Tree", function () {
				...
				if (typeof (NUTOOL_CLOCK.g_unusedModule) !== 'undefined' && typeof (NUTOOL_CLOCK.g_unusedModule[mcu]) !== 'undefined') {
					...
					for (k = 0, maxK = unusedObject.length; k < maxK; k += 1) {
						...
						for (j = 0, maxJ = localModuleProperties.length; j < maxJ; j += 1) {
							if (moduleName === localModuleProperties[j]) {
						...
						if (moduleName === 'LXT') {
							for (j = 0, maxJ = NUTOOL_CLOCK.g_register_map['PWRCTL'].length; j < maxJ; j += 1) {
								if (NUTOOL_CLOCK.g_register_map['PWRCTL'][j].indexOf('LXTEN' + ':') === 0) {
									NUTOOL_CLOCK.g_register_map['PWRCTL'].splice(j, 1);
									break;
								}
							}
						...
						if (moduleName === 'HXT') {
							for (j = 0, maxJ = NUTOOL_CLOCK.g_register_map['PWRCTL'].length; j < maxJ; j += 1) {
								if (NUTOOL_CLOCK.g_register_map['PWRCTL'][j].indexOf('HXTEN' + ':') === 0) {
									NUTOOL_CLOCK.g_register_map['PWRCTL'].splice(j, 1);
									break;
								}
							}
							for (i = 0, max = selectorNames.length; i < max; i += 1) {
								for (j = NUTOOL_CLOCK.g_CLKSEL[selectorNames[i]].length - 1; j >= 0; j -= 1) {
									if (NUTOOL_CLOCK.g_CLKSEL[selectorNames[i]][j].indexOf('HXT:') === 0 ||
										NUTOOL_CLOCK.g_CLKSEL[selectorNames[i]][j].indexOf('HXT/') === 0 ) {
										NUTOOL_CLOCK.g_CLKSEL[selectorNames[i]].splice(j, 1);
									}
								}
							}
						...
						if (moduleName === 'PLL') {
							delete NUTOOL_CLOCK.g_register_map['PLLCTL'];
							delete NUTOOL_CLOCK.g_register_map_default['PLLCTL'];
							delete NUTOOL_CLOCK.g_register_map_description['PLLCTL'];

							for (i = 0, max = selectorNames.length; i < max; i += 1) {
								for (j = NUTOOL_CLOCK.g_CLKSEL[selectorNames[i]].length - 1; j >= 0; j -= 1) {
									if (NUTOOL_CLOCK.g_CLKSEL[selectorNames[i]][j].indexOf('PLL:') === 0 ||
										NUTOOL_CLOCK.g_CLKSEL[selectorNames[i]][j].indexOf('PLL/') === 0 ) {
										NUTOOL_CLOCK.g_CLKSEL[selectorNames[i]].splice(j, 1);
									}
								}
							}
				...
				// retrieve the generated code to check HCLK or more.
